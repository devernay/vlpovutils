...
void vlpov_motionfield2(const VLPovCamera2 &cam1, const VLPovImage<double> &z1, const VLPovCamera2 &cam2, VLPovImage<double> &m1x, VLPovImage<double> &m1y)
{
    ...
    double fx = cam1.K(0,0);
    double fy = cam1.K(1,1);
    double s = cam1.K(0,1);
    double cx = cam1.K(0,2);
    double cy = cam1.K(1,2);
    ...
    for(int y1 = 0; y1 < z1.height(); ++y1) {
        for(int x1 = 0; x1 < z1.width(); ++x1) {
            ...
            double x1i = ifx*x1 + is*y1 + icx;
            double y1i = ify*y1 + icy;
...
#           endif
            // reconstruct in the global reference frame
            vector3 P = prod(trans(cam1.R),P1-cam1.t);
            // reconstruct in camera 2 reference frame
